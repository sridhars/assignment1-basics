...
def train_bpe_chunk(start: int, end: int, input_path, special_token_list: list[str]):
    special_escape_tokens = list(map(lambda x: re.escape(x), special_token_list))
...
def process_word(orig_word: tuple[bytes, ...], merge_pair: tuple[bytes, bytes], pairs_count, pairs_to_words : defaultdict[tuple[bytes, bytes], list[tuple[bytes, ...]]], freq: int):
...
def train_bpe(input_path: str, vocab_size: int, special_tokens: list[str]):
    ...
    while len(vocab) < vocab_size and len(pairs_count) > 0:
        ...
        for orig_word in matching_words:
            new_word = process_word(orig_word, merge_pair, pairs_count, pairs_to_words, pretoken_counts[orig_word])
...
def main():
    eot_string = "<|endoftext|>"
    vocab, merges = train_bpe("./data/TinyStoriesV2-GPT4-valid.txt", 1000, [eot_string])
